...
**Hook gap:** the `PostToolUse` validation hook (`hooks/doc-validate.sh`) resolves `docs_dir` from `.scribe.yml`'s `output.docs_dir` only — it does not know about the `branch-local` strategy's `.scribe/branch-docs/` override, so under `branch-local` the hook validates against the configured `docs_dir` path, not the branch-local output location. This is a known gap, not planned to be fixed. (Test-only: the hook extracts `.tool_input.file_path` through three rungs, strongest parser first — `jq`, then python, then an awk JSON scanner that validates the whole payload and selects the key path itself. `SCRIBE_NO_JQ=1` skips the first rung and `SCRIBE_NO_PYTHON=1` skips the second, so every rung is reachable deterministically on a machine that has all three; neither variable is meant for normal use. `hooks/test-doc-validate.sh` runs its whole case list three times: no overrides, then `SCRIBE_NO_JQ=1`, then both variables set. The third pass forces the awk scanner, so that rung is exercised on every machine; which rung the first two reach depends on what is installed, so the harness resolves it per pass, names it in that pass's banner, and prints a closing line for any rung the machine could not exercise — a tripled pass count is never mistaken for three-parser coverage.)
