...
It does not read `.scribe.yml`, does not scan `src/`, and does not read
`AGENTS.md` even when present.
...
- **HARD RULE 3 (no scanning)** — cases 001, 002, 003 and 005 include a distractor
  `src/` tree with a distinctive identifier; `no_codebase_leakage` fails if that
  identifier appears in a stub body. Case-004 (collision refusal) ships no `src/`
  tree but still contributes: its `forbidden_leakage_strings` names
