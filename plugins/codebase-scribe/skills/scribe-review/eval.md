...
Read/Bash/Grep/Glob access, so `src/` in each case is real source, not just
described in the brief — the agent's mandate is "do not trust the brief alone
...
- **Seeded documentation errors named in the task brief**:
  ...
  - case-003 seeds **deprecated-as-current** documentation (`EvaluateFlag` in
    `src/internal/flags/evaluator.go` — a live, compiling function carrying a
