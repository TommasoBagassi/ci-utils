...
6. **Update freshness only.** Set `freshness: 100`; do NOT recalculate `human_input` or `completeness`. Under `main-only` when `current_branch` != `default_branch`, do not update `freshness` or `scan`; the No-HEAD rule applies. Rework never reaches §10, so §10's preservation clause applies here in full — every other frontmatter key survives verbatim.
...
2. **On an answer:** incorporate it per §6/§7's incorporation-target rule, update `human_sections`/`inferred_sections` per HARD RULE 4, recompute `human_input` (scribe-lib). Extract the claim and write the `decisions:` entry at §11 timing.
3. **Increment `question_passes` on every question pass over the topic** — answered, skipped, or zero-question (otherwise a conventional topic would classify `unverified` and invoke a no-op pass forever). Write the topic file per §10 with the updated counter.
...
9f normally resets `question_passes` on draft/rework. Two paths bypass 9f, so a normal draft or rework — never a question pass — applies the same reset directly: **`review.enabled: false`** (at §10's write) and **the 9b-skip path** (after Step 9 returns, for the skipped topic). Either way: reset to 0 unless `question_passes == 2` with `human_input == 0` — 9f's condition (b), carried unchanged.
...
4. **ALWAYS update `human_sections` and `inferred_sections` when incorporating user answers.** Add the receiving section's top-level slug to `human_sections` (the scoring list) AND remove it from `inferred_sections`. Top-level parent only, never subsection slugs. `human_sections` is a set: re-adding an existing slug is a no-op — repeat credits must not inflate the score.
...
Each outcome writes the frontmatter `decisions:` entry **first**, then `.claims.yml`, then (where applicable) topic content and the stale flag. Every outcome stamps `resolved_at: <current HEAD>` on the entry — what lets maintain §4 stop re-flagging a resolved decision. This section is reachable when HEAD cannot be read (a flag from an earlier git-available run survives): there, write no `resolved_at` at all — maintain §4's base selection falls back to `scan` for an entry without it, failing toward re-detection; every other write still applies.

- **Option 1 (still valid):** update the entry's `recorded` date, stamp `resolved_at`; update the claim's `provenance.recorded` to match; remove the stale flag.
- **"Other" (updated reasoning):** update the entry's `context` and `recorded`, stamp `resolved_at`; update the claim's `context`/`recorded`; update the topic content; remove the stale flag. The updated reasoning is an incorporated user answer — HARD RULE 4 applies (add slug to `human_sections`, remove from `inferred_sections`, recompute `human_input` in the same write). Under `questions: false` this is the only path that can raise `human_input`.
- **Option 2 (retire):** set the entry's `status: retired` — a tombstone, skipped by re-linking (maintain §6) and drift detection (maintain §4) — stamp `resolved_at`; remove the claim from `.claims.yml` (add its ID to `_retired_ids`); remove related topic content — **except prose in a `human_sections` section, which is preserved per Safety Rule 2: remove the claim only.** The tombstone survives on every path (it reserves the ID and keeps re-linking away); no outcome deletes a `decisions:` entry. No path deletes credited human prose, so `human_sections` and `human_input` are untouched by this outcome.
...
5. After writing, **remove `migration_source` and `migration_sections`** from frontmatter — consumed
...
Write the complete topic file: YAML frontmatter (scan = current HEAD — only on the default branch under `main-only`, not in a question pass, and subject to the No-HEAD rule; scores; `inferred_sections`; `watch_paths` restamped from the brief's repaired value — never narrowed by draft; empty `stale_flags` on full drafts only — a question pass carries existing `stale_flags` through) and the markdown content per §3.

**Preservation clause (canonical list):** every key not named by the writing rule survives verbatim — `decisions`, `question_passes`, `human_sections`, `review_notes`, and any other keys present (HARD RULE 4 is `human_sections`' named writer).
...
**Write the `decisions:` entry:** immediately after a `origin: user` claim is extracted and assigned its id, write (or update) the frontmatter `decisions:` entry whose `id` equals the claim's — a `status: retired` entry is never updated or reactivated. This is where the entry is written; §6/§7 only record the answer. The entry mirrors the claim's `id`/`type`/`claim`/`source` and the provenance's `context`/`recorded`, with `status: active`.

When a claim is deleted (e.g., decision drift resolution), add its ID to `_retired_ids`.

Append claims to `.claims.yml` in docs_dir. Include `_meta` with the topic's current git SHA as `<topic>_extracted_at`.
...
Present all queued non-critical questions. Document answers into the relevant topics using §6's incorporation-target rule. This pass runs *after* §8/§10 wrote the files, so for each answered section: add the top-level slug to `human_sections`, remove it from `inferred_sections`, recompute `human_input` (scribe-lib), and rewrite that topic's frontmatter — a partial update changing those three keys and nothing else (§10's preservation clause governs; named here because this pass reaches the file after §10, not through it).
