...
Shared definitions — **Repo root = cwd**, **scribe-lib**, **Threaded fields**, **Maturity test**, **No-HEAD rule**, **partial frontmatter updates** — live in `commands/codebase-scribe.md` § Definitions and apply here as written. Every brief carries the Threaded fields: use the passed values, never re-derive them. The repo being maintained is ALWAYS the current working directory: every reference check and every frontmatter or STATUS.md write resolves against cwd — never against a plugin directory or any other project root visible in context.
