...
**Section credit, deterministic:** add the target section's slug to `human_sections` only when exactly one `##` section's body contains the claim text; otherwise record the decision without section credit. When a slug is added, recompute `human_input` (scribe-lib `human-input`) and persist it in the same write — Step 5's `unverified` row reads the stored score, and a topic that just earned credit must not route into Question-Pass Mode. A migration adding no slug recomputes nothing.

This is a partial frontmatter update: `decisions:`, and where a slug matched, `human_sections` and `human_input` — nothing else. The staged untrack and any `.gitignore` modification are reported in the Step 13 summary with an instruction to commit.
...
- `<topic>.headings.txt` — the topic's `##` heading list (scribe-lib `sections`, heading text one per line)
...
3. Update `scan` to current HEAD — only for topics whose content was drafted or reworked this run. A question pass and a topic passed over for an unanswered decision-drift prompt are neither, and never stamp either field — that is what keeps unresolved decision drift out of the new baseline. Under `main-only` when `current_branch` != `default_branch`, do not update `freshness` or `scan`. The No-HEAD rule applies.
4. Update `freshness: 100` — same conditions as item 3. After a maintain-only pass, preserve the freshness maintain §8 computed and do not advance `scan`.
5. Reset `question_passes` to 0 — both required: (a) the topic was drafted or reworked this run (item 3's predicate — question-pass output classified `major_rewrite` by line count does NOT qualify), AND (b) NOT (`question_passes == 2` with `human_input == 0`). A user who declined twice stays settled even across genuine redrafts; without (b), every churny redraft would re-open two more passes indefinitely.
